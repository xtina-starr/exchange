...
describe CreateBuyOrderService, type: :services do
  describe '#process!' do
    let(:user_id) { 'user-id' }
    ...
    context 'known artwork' do
      ...
      context 'without edition set' do
        ...
        let(:service) { CreateBuyOrderService.new(user_id: user_id, artwork_id: 'artwork-id', edition_set_id: nil, quantity: 2) }
      ...
      context 'artwork with one edition set' do
        ...
        context 'with passing edition_set_id' do
          let(:service) { CreateBuyOrderService.new(user_id: user_id, artwork_id: 'artwork-id', edition_set_id: 'edition-set-id', quantity: 2) }
          it 'creates order' do
            expect do
              service.process!
              order = service.order
        ...
        context 'without passing edition_set_id' do
          let(:service) { CreateBuyOrderService.new(user_id: user_id, artwork_id: 'artwork-id', quantity: 2) }
    ...
    context 'with unknown artwork' do
      ...
      it 'raises error' do
        expect { CreateBuyOrderService.new(user_id: user_id, artwork_id: 'random-artwork', quantity: 2).process! }.to raise_error do |error|
          expect(error).to be_a(Errors::ValidationError)
