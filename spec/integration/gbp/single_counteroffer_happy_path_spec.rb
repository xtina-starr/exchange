...
describe Api::GraphqlController, type: :request do
  ...
  describe 'make offer happy path' do
    ...
    let(:seller_addresses) do
      [Address.new(city: 'London', country: 'GB', postal_code: 'SW3 4RY')]
    end
    ...
    it 'succeeds in the process of buyer offers -> seller counters -> buyer accepts' do
      ...
      expect do
        seller_client.execute(
          OfferQueryHelper::SELLER_COUNTER_OFFER,
          input: { offerId: offer.id.to_s, amountCents: 700_00 }
        )
      end.to change(order.transactions, :count).by(0).and change(
                                                    order.offers,
                                                    :count
                                                  ).by(1)
      ...
      expect do
        buyer_client.execute(
          OfferQueryHelper::BUYER_ACCEPT_OFFER,
          input: { offerId: seller_counter.id.to_s }
        )
