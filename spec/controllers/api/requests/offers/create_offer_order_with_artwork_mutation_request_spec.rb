    ...
    context 'with proper token' do
      ...
      context 'with failed artwork fetch' do
        ...
        it 'does not create order and returns proper error' do
          expect do
            ...
            expect(
              response.data.create_offer_order_with_artwork.order_or_error
            ).not_to respond_to(:id)

            expect(
              response
                .data
                .create_offer_order_with_artwork
                .order_or_error
                .error
                .type
            ).to eq 'validation'
            ...
            it 'creates a new offer order' do
              expect do
                response =
                  client.execute(
                    mutation,
                    input: { artworkId: artwork_id, findActiveOrCreate: false }
                  )
                expect(
                  response
                    .data
      ...
      context 'with not-offerable artwork' do
        before do
          expect(Gravity).to receive(:get_artwork)
            .with(artwork_id)
            .and_return(gravity_v1_artwork(edition_sets: nil, offerable: false))
        end
        it 'returns error' do
          expect do
            response =
