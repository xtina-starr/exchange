...
describe Api::GraphqlController, type: :request do
  describe 'buyer_counter_offer mutation' do
    ...
    let!(:order) { Fabricate(:order, mode: Order::OFFER, state: order_state, seller_id: order_seller_id, buyer_id: buyer_id, items_total_cents: 421, **shipping_info) }
    let!(:seller_offer) { Fabricate(:offer, order: order, amount_cents: 10000, from_id: order_seller_id, from_type: 'gallery', submitted_at: Time.now.utc) }
    ...
    context 'when not in the submitted state' do
      ...
      it "returns invalid state transition error and doesn't change the order state" do
        ...
        expect(response.data.buyer_counter_offer.order_or_error.error.type).to eq 'validation'
        expect(response.data.buyer_counter_offer.order_or_error.error.code).to eq 'invalid_state'
    ...
    context 'when attempting to counter not-the-last-offer' do
      it 'returns a validation error and does not change the order state' do
        create_another_offer(from_id: order.seller_id, from_type: order.seller_type)
        response = client.execute(mutation, buyer_counter_offer_input)
        expect(response.data.buyer_counter_offer.order_or_error.error.type).to eq 'validation'
    ...
    context 'with user without permission to order' do
      ...
      it 'returns permission error' do
        ...
        expect(response.data.buyer_counter_offer.order_or_error.error.type).to eq 'validation'
        expect(response.data.buyer_counter_offer.order_or_error.error.code).to eq 'not_found'
        expect(order.reload.state).to eq Order::SUBMITTED
      end
    end
    ...
    context 'when the specified offer does not exist' do
      ...
      it 'returns a not-found error' do
        expect { client.execute(mutation, buyer_counter_offer_input) }.to raise_error do |error|
    ...
    context 'when not waiting for buyer response' do
      before do
        seller_offer.update!(from_id: order.buyer_id, from_type: order.buyer_type)
      end
      it 'returns cannot_counter' do
        response = client.execute(mutation, buyer_counter_offer_input)

        expect(response.data.buyer_counter_offer.order_or_error.error.type).to eq 'validation'
        expect(response.data.buyer_counter_offer.order_or_error.error.code).to eq 'cannot_counter'
      end
    end
    ...
    context 'with proper permission' do
      ...
      it 'counters the order' do
        expect do
          ...
          expect(last_pending_offer.amount_cents).to eq(10000)
      ...
      context 'with offer note' do
        ...
        it 'counters the order with note' do
          client.execute(mutation, buyer_counter_offer_input)
          last_pending_offer = order.offers.where(from_id: jwt_user_id).order(created_at: :desc).first
          expect(last_pending_offer.offer_note).to eq(offer_note)
        end
      end
    end
  end
  ...
  def create_another_offer(from_id:, from_type:)
    another_offer = Fabricate(:offer, order: order, from_id: from_id, from_type: from_type)
