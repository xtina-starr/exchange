RSpec.shared_context 'include stripe helper' do
  ...
  def prepare_payment_intent_create_failure(
    id: 'pi_1',
    ...
    when 'requires_payment_method'
      error =
        Stripe::CardError.new(
          ...
          decline_code: charge_error[:decline_code],
          code: charge_error[:code]
        )
      allow(error).to receive(:json_body).and_return(
        error: {
          payment_intent:
            basic_payment_intent(
              status: status,
              capture: capture,
              amount: amount,
              code: charge_error[:code],
  ...
  def prepare_payment_intent_create_success(
    capture: false,
    payment_method: 'cc_1',
    amount: 20_00,
    id: 'pi_1'
  ...
  def prepare_payment_intent_confirm_failure(
    id: 'pi_1',
    charge_error:,
    payment_method: 'cc_1',
    amount: 20_00,
    ...
    payment_intent =
      double(
        ...
        last_payment_error: double(charge_error)
      )
    error =
      Stripe::CardError.new(
        charge_error[:message],
  ...
  def prepare_payment_intent_confirm_raise_invalid(
    payment_method: 'cc_1',
    amount: 20_00,
  ...
  def prepare_payment_intent_confirm_success(
    id: 'pi_1',
    payment_method: 'cc_1',
    amount: 20_00
  )
    ...
    allow(payment_intent).to receive(:status).and_return(
  ...
  def prepare_payment_intent_capture_failure(
    charge_error:,
    payment_method: 'cc_1',
    amount: 20_00
  )
  ...
  def prepare_payment_intent_capture_success(
    payment_method: 'cc_1',
    amount: 20_00
  )
    payment_intent =
  ...
  def prepare_payment_intent_capture_update_transfer_data_success(
    payment_method: 'cc_1',
    amount: 20_00,
    transfer_amount: 20_00
  )
    payment_intent =
      double(
        id: 'pi_1',
        ...
        amount: amount,
        capture_method: 'manual',
        transfer_data: double(destination: 'ma_1', amount: transfer_amount)
      )
    allow(payment_intent).to receive(:status).and_return(
      'requires_capture',
      'succeeded'
    ...
    allow(payment_intent).to receive(:capture)
    allow(payment_intent).to receive(:to_h).and_return(
      :id => 'pi_1',
      :client_secret => 'pi_test1',
  ...
  def prepare_payment_intent_cancel_failure(
    charge_error:,
    payment_method: 'cc_1',
    amount: 20_00
  )
  ...
  def prepare_payment_intent_cancel_success(
    payment_method: 'cc_1',
    amount: 20_00
  )
    payment_intent =
  ...
  def prepare_payment_intent_refund_success(
    payment_method: 'cc_1',
    amount: 20_00
  )
    payment_intent =
    ...
    allow(Stripe::Refund).to receive(:create)
      .with(charge: 'ch_1', reverse_transfer: true)
      .and_return(refund)
    mock_payment_intent_call(:retrieve, payment_intent)
  end

  def prepare_payment_intent_refund_failure(
    charge_id: 'ch_1',
    payment_method: 'cc_1',
    amount: 20_00,
    code:,
    ...
    allow(refund_error).to receive(:json_body).and_return(
      error:
        stripe_exception_json_body(
          charge_id: charge_id,
          code: code,
    ...
    allow(Stripe::Refund).to receive(:create)
      .with(charge: 'ch_1', reverse_transfer: true)
      .and_raise(refund_error)
    mock_payment_intent_call(:retrieve, payment_intent)
  end

  def prepare_setup_intent_create(payment_method: 'cc_1', status: 'succeeded')
    setup_intent =
      double(id: 'si_1', payment_method: payment_method, status: status)
    allow(setup_intent).to receive(:to_h).and_return(
      id: 'si_1',
      client_secret: 'si_test1'
    )
  ...
  def prepare_setup_intent_retrieve(
    payment_method: 'cc_1',
    status: 'succeeded',
    on_behalf_of: 'acc_123'
  )
    setup_intent =
      double(
        id: 'si_1',
  ...
  def basic_payment_intent(
  ...
  )
    {
        ...
        data: [
          {
            ...
            receipt_number: nil,
            receipt_url:
              'https://pay.stripe.com/receipts/acct_172ojZGK3Gnpfa3O/ch_1/rcpt_FVWPR1qfouqPv2nnbYnzRqY9RyvVu76',
            refunded: false,
            refunds: {
              object: 'list',
              data: [],
      ...
      statement_descriptor: nil,
      status: status.to_s,
      transfer_data: { destination: 'ma_1' },
      transfer_group: nil
    }
  end
  ...
  def basic_setup_intent(code: 'card_declined', decline_code: 'do_not_honor')
    {
      ...
      cancellation_reason: nil,
      client_secret:
        'seti_1FJEY2GK3Gnpfa3OloLFhpiV_secret_FosI3jo5pM0sXWNtJcK1YfGlqCuDq7F',
      created: 1_568_618_578,
