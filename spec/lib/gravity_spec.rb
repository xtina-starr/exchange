...
describe Gravity, type: :services do
  ...
  describe '#fetch_partner_locations' do
    ...
    let(:tax_only_params) do
      base_params.merge(address_type: ['Business', 'Sales tax nexus'])
    end
    ...
    context 'calls the correct location Gravity endpoint' do
      context 'without tax_only flag' do
        it 'does not filter by address type' do
          expect(Adapters::GravityV1).to receive(:get)
            .with("/partner/#{seller_id}/locations", params: base_params)
      ...
      context 'with tax_only flag' do
        it 'filters by address type' do
          expect(Adapters::GravityV1).to receive(:get)
            .with("/partner/#{seller_id}/locations", params: tax_only_params)
            .and_return(valid_locations)
  ...
  describe '#get_merchant_account' do
    ...
    context 'with merchant account' do
      before do
        allow(Adapters::GravityV1).to receive(:get)
          .with('/merchant_accounts', params: { partner_id: seller_id })
  ...
  describe '#get_credit_card' do
    ...
    it 'calls the /credit_card Gravity endpoint' do
      allow(Adapters::GravityV1).to receive(:get)
        .with("/credit_card/#{credit_card_id}")
  ...
  describe '#fetch_all' do
    it 'paginates until there are no more items' do
      allow(Adapters::GravityV1).to receive(:get)
        .with('/test', params: { page: 1, size: 20 })
        .and_return(
          [
            1,
            2,
      ...
      allow(Adapters::GravityV1).to receive(:get)
        .with('/test', params: { page: 2, size: 20 })
        .and_return(
          [
            1,
            2,
      ...
      allow(Adapters::GravityV1).to receive(:get)
        .with('/test', params: { page: 3, size: 20 })
        .and_return([1, 2, 3])
    ...
    let(:response) do
      {
        'data' => {
          'debitCommissionExemption' => {
            'amountOfExemptGmvOrError' => { 'fooBar' => 'baz' }
          }
        }
      }
    end
    it 'returns snake-cased amountOfExemptGmvOrError if it is included in the response body' do
      allow(GravityGraphql).to receive_message_chain(
        :authenticated,
        :debit_commission_exemption
      ).and_return(response)
  ...
  describe '#refund_commission_exemption' do
    it 'requests the credit commission exemption mutation and returns nil' do
      mutation =
        ...
        ).to_return(status: 200, body: { foo: { bar: 'baz' } }.to_json)
      retval =
