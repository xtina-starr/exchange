...
class ReminderFollowUpJob < ApplicationJob
  ...
  def perform(order_id, state)
    ...
    when Order::SUBMITTED
      PostNotificationJob.perform_now(order.id, Order::REMINDER_EVENT_VERB[:seller_delay])

    when Order::APPROVED
      PostNotificationJob.perform_now(order.id, Order::REMINDER_EVENT_VERB[:buyer_delay])
    end
  end
end
