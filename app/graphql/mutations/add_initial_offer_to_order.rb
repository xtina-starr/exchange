class Mutations::AddInitialOfferToOrder < Mutations::BaseMutation
  ...
  def resolve(order_id:, amount_cents:, offer_note: nil)
    order = Order.find(order_id)
    authorize_buyer_request!(order)
    raise Errors::ValidationError, :cannot_offer unless order.state == Order::PENDING

    OfferService.create_pending_offer(order, amount_cents: amount_cents, offer_note: offer_note, from_id: current_user_id, from_type: Order::USER, creator_id: current_user_id)
